...
&emsp;注意：这里 *a* 的类型是右值引用类型(*int &&*)，但是如果从左值和右值的角度区分它，它实际上是个左值。因为可以对它取地址，而且它还有名字，是一个已经命名的右值。
...
&emsp;为了更好的观察结果，可以在编译的时候加上 *-fno-elide-constructors* 选项(关闭返回值优化)。
```cpp
...
<img src="./copy和move的区别.png" height="100" width="180">
